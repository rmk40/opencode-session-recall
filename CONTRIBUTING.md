...
| `store.ts`                   | SQLite card + slim-FTS store: schema v3, additive migrations (v1→v2→v3)/rebuild, per-session transactional replace, FTS query, card-vector persistence with per-row generations, lease |
...
| `sessions.ts`                | `recall_sessions` tool (card-backed enrichment, since/until, live `parentID` children lookup, staleness fallback)                                                                      |
...
Off unless the `semantic` plugin option is set, and it operates at the **card tier**, not the part level. `cards.ts` embeds each card's text once and blends the cosine similarity between the query vector and each card vector into that card's lexical score (weighted by `semanticWeight`). It is a mild reorderer, not a vocabulary-gap solver: the embedded text is a natural-language projection of the card (`embedding-text.ts`, identifiers split into words), not raw fields, but the default model is small, so real-corpus testing found the blend only nudges borderline paraphrases at the margin, and rephrasing the query stays the reliable path when wording diverges. Card vectors persist lazily to the `card.embedding` column, stamped in `meta.semantic_model` with both the model id and the embedding representation generation (`EMBED_REPRESENTATION`), and each row also carries its own `embedding_gen`; a later process start reuses a vector when the stamp matches, the row still has a vector, and its generation is current, and recomputes otherwise (a re-distilled card is upserted with a null embedding, so staleness self-heals). A lower-generation writer can never downgrade a higher one, and a change to the representation semantics rides a `SCHEMA_VERSION` bump. Final result ranking stays lexical over the drilled parts, so a session the embedding surfaced but whose drilled parts yield no lexical hit simply drops from the results. Any failure (model missing, embed error) leaves ranking exactly as the lexical path produced it, with one warning while the model is still loading. `semantic/embedder.ts` is the only module allowed to touch Node APIs, exclusively via dynamic `import()`, so non-Node runtimes and disabled installs degrade gracefully.
